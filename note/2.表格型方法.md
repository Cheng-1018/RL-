...
TD(0):V(s_t)&\leftarrow V(s_{t})+\alpha (G_{t}^{1}-V(s_{t})) \\
...
## 总结
比较一下 $Q$ 学习 和 $Sarsa$ 的更新公式，就可以发现 $Sarsa$ 并没有选取最大值的最大化操作。因此， $Q$ 学习是一个非常激进的方法，它希望每一步都获得最大的利益；$Sarsa$ 则相对较为保守，它会选择一条相对安全的迭代路线。
