...
$$
    \begin{aligned}
    V_{\pi}(s_{t})
    &=\gamma V_{\pi}(s_{t+1})+r_{t} \\
    V_{\pi}(s_{t})-\gamma V_{\pi}(s_{t+1})
    &\longleftrightarrow r_{t}
    \end{aligned}
$$

用 $V_{\pi}(s_{t})-\gamma V_{\pi}(s_{t+1})$ 去拟合 $r_{t}$
...
$T>0$ 称为温度系数， $T$ 很大，所有动作几乎以等概率选择，如果T很小， $Q$ 值大的动作容易未选中， $T$ 趋于0，只选择最优动作。

### 经验回放

经验回访会构建一个回放缓冲区，把与环境交互的数据存储在缓冲区，每一笔数据包含 $(s_{t},a_{t},r_{t},s_{t+1})$ ，这些数据可能来自于不同的策略。训练时从缓冲区随机挑出一个批量出来，用这些数据去更新 $Q$ 函数，这是一种异策略的方法
